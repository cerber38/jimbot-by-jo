...
package ru.jimbot.modules.chat.tasks;
...
public class ChangeStatusTask implements Task {
     ...
     * @param aw
     */
    public ChangeStatusTask(Service srv, long period, ChatWork cw) {
        this.srv = srv;
        this.period = period;
        this.cw = cw;
    }
...
//        System.out.println("exec ChangeStatusTask");
        if(cw==null) {
            ...
            String[] s = cw.getX_staus();

            if(s.length==0) return;
            srv.createEvent(new CommandProtocolChangeXStatusEvent(srv, Integer.parseInt(s[0]), s[1], s[2]));
          //   System.out.println("getX_staus "+s[0]+" "+s[2]);
        }
        countStart++;
    }
