...
public class ChatQueue implements Runnable {
    ...
    public ChatQueue(ChatService s) {
        srv = s;
        psp = s.getConfig().getInstance(srv.getName());
  ...
  /**
     ...
     */
    public void addUser(Message m, int room) {
        Log.getLogger(srv.getName()).debug("CHAT: Add user "+m.getSnIn() + ", " + m.getSnOut());
        uq.put(m.getSnIn(),new UinElement(m.getSnIn(),m.getSnOut(),0,room));
    }
    ...
    //	((ChatCommandParser) srv.getCommandParser()).testState(uin);
    ...
    public synchronized void send(){
    	try {
			...
			for (String c : uq.keySet()) {
				...
				((ChatCommandParser) srv.getCommandParser()).testState(c);
				if (!s.equals("") && uq.containsKey(c))
                                sendMsg(new Message(uq.get(c).baseUin, uq.get(c).uin,s));
			}
