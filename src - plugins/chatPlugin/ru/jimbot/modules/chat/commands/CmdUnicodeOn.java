 ...
 *
 */
public class CmdUnicodeOn extends DefaultCommand {
    public CmdUnicodeOn(Parser p) {
        super(p);
    }
    ...
    public List<String> getCommandPatterns() {
        return Arrays.asList(new String[] {"!вкл","!vkl"});
    }
...
//    }
   ...
   /**
    ...
    public String getHelp() {
        return " - включить поддержку символов Unicode";
    }
    ...
    public String exec(String sn, Vector param) {
       ChatWork uw =((ChatService)p.getService()).getChatWork();
        try{
            ...
            us.unicode=1;
            uw.updateUser(us);
           return "Поддержка символов Unicode включена!";
       } catch (Exception ex) {
         ex.printStackTrace();
