 ...
 */
    public Cookie[] getCookies(){
       ...
       for(Enumeration e=getHeaders("cookie"); e.hasMoreElements();){
           String element = (String)e.nextElement();
           v.add(new Cookie(element.split("=")[0], element.split("=")[1]));
       }
       if (v.size()>0) ñookie=(Cookie[]) v.toArray();  
        return ñookie;
    }
    
/** 
 * @return HashMap<String,Cookie>
 */
    public HashMap<String,Cookie> getHashCookies(){
        HashMap<String,Cookie> cookies = new HashMap<String,Cookie>();
       for(Enumeration e=getHeaders("cookie"); e.hasMoreElements();){
           String cookie[] = ((String)e.nextElement()).split("; ");
             for(int i=0; i<cookie.length; i++){
               cookies.put(cookie[i].split("=")[0],new Cookie(cookie[i].split("=")[0], cookie[i].split("=")[1]));
             }
        }
        return cookies;
    }
